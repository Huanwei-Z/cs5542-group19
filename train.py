...
def splitimage(frame, i):
    face_class = cv2.CascadeClassifier(
        r'F:\Python\Python36\Lib\site-packages\cv2\data\haarcascade_frontalface_default.xml')
...
def train_model(train_path, test_path):
    ...
    for i in range(len(lst_result)):
        file.write(('image%s' % lst_path[i]) + ('recognized as cow No.%d' % lst_result[i]) + '\n')
